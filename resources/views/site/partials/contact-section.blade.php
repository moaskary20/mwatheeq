            ...
            <div class="grid lg:grid-cols-2 lg:min-h-[620px]">
                <div class="contact-form-panel relative space-y-7 p-6 sm:p-8 lg:p-10">
                    <div class="reveal contact-hero-row">
                        ...
                        <div>
                            <p class="text-sm font-extrabold tracking-wide text-brand">{{ __('site.contact.ready_title') }}</p>
                            <p class="mt-2 text-sm leading-7 text-brand/70">
                                {{ __('site.contact.ready_lead') }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="reveal contact-info-grid" style="transition-delay: 60ms">
                        @if ($settings['address'])
                            <div class="contact-info-card">
                                ...
                                <div>
                                    <p class="contact-info-label">{{ __('site.contact.address') }}</p>
                                    <p class="contact-info-value">{{ is_arabic() ? $settings['address'] : ($settings['address_en'] ?: $settings['address']) }}</p>
                                    @if (is_arabic() && ! empty($settings['address_en']))
                        ...
                        @if ($settings['email'])
                            <a class="contact-info-card contact-info-card--link" href="mailto:{{ $settings['email'] }}">
                                ...
                                <div>
                                    <p class="contact-info-label">{{ __('site.contact.email') }}</p>
                    ...
                    @if ($hasSocial)
                        <div class="reveal social-links" style="transition-delay: 80ms">
                            <p class="social-links-label">{{ __('site.contact.social') }}</p>
                            <div class="social-links-row">
                                @if ($facebookUrl)
                                    <a class="social-btn social-btn--facebook" href="{{ $facebookUrl }}" target="_blank" rel="noopener" aria-label="{{ __('site.contact.facebook') }}">
                                ...
                                @if ($instagramUrl)
                                    <a class="social-btn social-btn--instagram" href="{{ $instagramUrl }}" target="_blank" rel="noopener" aria-label="{{ __('site.contact.instagram') }}">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5" aria-hidden="true">
                    ...
                    <form action="{{ route('contact.store') }}" method="POST" class="reveal contact-form" style="transition-delay: 100ms">
                        ...
                        <div class="contact-form-head">
                            ...
                            <p>{{ __('site.contact.form_lead') }}</p>
                        </div>

                        <div>
                            <label for="name" class="field-label">{{ __('site.contact.name') }}</label>
                            <input id="name" name="name" type="text" value="{{ old('name') }}" required class="field" placeholder="{{ __('site.contact.name_ph') }}">
                            @error('name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <label for="email" class="field-label">{{ __('site.contact.email_label') }}</label>
                                <input id="email" name="email" type="email" value="{{ old('email') }}" required class="field" placeholder="{{ __('site.contact.email_ph') }}" dir="ltr">
                                @error('email') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                            <div>
                                <label for="phone" class="field-label">{{ __('site.contact.mobile') }}</label>
                                <input id="phone" name="phone" type="text" value="{{ old('phone') }}" class="field" placeholder="{{ __('site.contact.mobile_ph') }}" dir="ltr">
                                @error('phone') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                            </div>
                        </div>
                        <div>
                            <label for="subject" class="field-label">{{ __('site.contact.subject') }}</label>
                            <input id="subject" name="subject" type="text" value="{{ old('subject') }}" class="field" placeholder="{{ __('site.contact.subject_ph') }}">
                            @error('subject') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="message" class="field-label">{{ __('site.contact.message') }}</label>
                            <textarea id="message" name="message" rows="5" required class="field field-textarea" placeholder="{{ __('site.contact.message_ph') }}">{{ old('message') }}</textarea>
                            @error('message') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <button type="submit" class="btn-primary contact-submit">
                            <span>{{ __('site.contact.submit') }}</span>
                ...
                <div class="map-panel relative min-h-[420px] bg-brand lg:min-h-full">
                    ...
                    <div class="map-overlay-top">
                        ...
                        <span>{{ __('site.contact.map_badge') }}</span>
                    </div>
